...
func TestRangesNatively(t *testing.T) {
	...
	assert.True(t, backend.RangesNatively(ctx, backendtest.NewByteCounter(backend.Memory()), "k"))
	assert.False(t, backend.RangesNatively(ctx, backendtest.NewByteCounter(bare), "k"))
}
