...
func (bb *bloomBuilder) forEachGram(values *cells, fn func(gram []byte)) {
	bb.gram.ext.MinLen, bb.gram.ext.MaxLen = gramMinLen, gramMaxLen

	bb.each(values, func(v []byte) {
...
func (bb *bloomBuilder) distinctGrams(values *cells) int {
	bb.distinct.Reset()
...
// once to fill — rather than materializing every gram to learn the count. The column is taken in
// either the flat or the split form ([cells]); the filter is identical either way.
func (bb *bloomBuilder) buildGrams(values cells) []byte {
	bb.view = values
	v := &bb.view
	...
	f := bloom.New(bb.distinctGrams(v), gramFalsePositiveRate)
	bb.forEachGram(v, f.Add)
...
func buildColumnGrams(values *byteCol) []byte {
	...
	return bb.buildGrams(cells{flat: values})
}
...
func writeGrams(
	...
	for k := range schema.byteCols {
		...
		data := bb.buildGrams(cols.cellsAt(k))
		if data == nil {
			continue
		}
