...
// TestRepairCoveredWantIsNotAFailure is the record-signal twin of the metric engine's test for
// issue #577: two wants answered by one merged successor arrive as two entries when the peer names
// each separately, and the second names a copy nothing brought in. Opening it fails, but the want
// it stands for is already discharged by the successor committed alongside it — nothing failed and
...
func TestRepairCoveredWantIsNotAFailure(t *testing.T) {
	...
	dropObjects(t, be, first)
	dropObjects(t, be, second)
	...
	const phantom = "t/recs/00000000000000000000000000"
	...
	assert.Empty(t, committedIndex(t, be).Wanted)
	assert.ElementsMatch(t, []string{"p1", "p2", "p3"}, streamBodies(t, e),
		"no records are lost or double-counted")
}
