...
//
// Part prefixes are minted and never reused (see [Engine.newPartPrefix]), so a cached entry cannot
// be served for a *different* part that later took the same key; entries for deleted parts are just
// garbage the policy ages out.
