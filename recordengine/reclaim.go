...
// counting, so a lock-free reader never races a delete. The single-mutator invariant (only the
// background maintenance task, never two at once, mutates `parts`) keeps the swaps simple.
...
// replaceParts returns a new slice with every part whose prefix is in removed dropped and each non-nil
// add appended (add may be empty, e.g. a retention merge that produced no part). Copy-on-write. It
// keeps any part not in removed, so a part added concurrently is never lost.
...
func partRows(parts []*part) int64 {
	var n int64
