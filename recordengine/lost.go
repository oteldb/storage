...
func partCorrupt(err error) bool {
	...
	return errors.Is(err, block.ErrCorrupt) || errors.Is(err, ErrCorruptKeys)
}
...
func (e *Engine) countCorrupt(ctx context.Context, err error, component, disposition string) {
	if errors.Is(err, block.ErrCorrupt) || errors.Is(err, bucketindex.ErrCorrupt) || errors.Is(err, ErrCorruptKeys) {
