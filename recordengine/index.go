...
// no-op. Replaces current parts. It assumes this node owns the prefix — it sweeps the part objects
// the index does not name (see [Engine.sweepOrphansLocked]) and commits a want for each part the
// index names that is not there.
...
func (e *Engine) loadPartsLocked(ctx context.Context, mode loadMode) error {
	...
	if err != nil {
		...
		e.cfg.Obs.Corruption.FencedLoad(ctx, e.cfg.Signal, fenceReason(err))
...
func (e *Engine) RefreshReplica(ctx context.Context) error {
	...
	for _, p := range e.parts {
		...
		for _, w := range wmarks {
			if t, ok := covered[w.ID]; !ok || w.Max > t {
				covered[w.ID] = w.Max
			}
		}
	}
