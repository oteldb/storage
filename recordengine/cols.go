...
// column cannot be permuted in place, so each needs a destination; swapping the result with the
// column's old array hands that array on as the next column's destination, so a whole flush
// allocates one buffer instead of one per byte column per unsorted stream. A nil scratch allocates.
