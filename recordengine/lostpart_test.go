...
func twoParts(t *testing.T, e *recordengine.Engine, be backend.Backend) []string {
	t.Helper()
...
func TestGonePartBecomesWant(t *testing.T) {
	...
	require.Equal(t, []string{"p2"}, streamBodies(t, r))

	ix := loadIndex(t, be)
	require.Equal(t, []string{enginePrefix + "/" + ids[1]}, prefixes(ix.Entries))
	require.Equal(t, []string{enginePrefix + "/" + ids[0]}, wantPrefixes(ix.Wanted))
...
// TestPartiallyGonePartBecomesWant covers the half-erased part: the manifest is there and the
// column data is not. It cannot answer a read, so it is as lost as one with no objects at all — and
...
func TestCorruptPartIsNotAWant(t *testing.T) {
	...
	r := newEngine(t, be)
	require.Error(t, r.LoadParts(ctx))
	require.Empty(t, loadIndex(t, be).Wanted)
}
...
func TestWantIsRecordedInOneCommit(t *testing.T) {
	...
	require.Contains(t, wantPrefixes(ix.Wanted), enginePrefix+"/"+ids[0])
}
...
func TestWantSurvivesLaterCommits(t *testing.T) {
	...
	ingest(t, r, mkBatch("api", rrec{ts: 300, body: "p3"}))
...
// commit path: random ingest, flush, merge, retention and part-loss sequences, with every committed
	...
	for seed := range uint64(24) {
		t.Run("", func(t *testing.T) {
			...
			for step := range 12 {
				...
				switch rng.IntN(5) {
				case 0, 1:
					ingest(t, e, mkBatch("api", rrec{ts: ts, body: "r"}))
					require.NoError(t, e.Flush(ctx))
				case 2:
