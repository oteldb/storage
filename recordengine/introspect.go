...
// bounds, and the stream/row counts from the part's in-memory row-range index.
...
// streams). TotalSeries and SymbolCount are exact; Top is the highest-cardinality label names.
...
// LabelCard is one label name's cardinality: how many streams carry it and how many distinct values
...
// and chunk (granule) count. It reads object sizes from the backend, so unlike Parts it is not
...
// which no merge reconsiders. It is [MergeShape.Backlog]; use [Engine.MergeShape] for the rest of
...
// epoch. ok is false when the engine has no WAL (ephemeral). It takes a read lock, excluding
// concurrent appends (which hold the write lock).
...
// every stream ever seen, flushed or not). topN bounds the returned Top slice (≤0 returns all label
