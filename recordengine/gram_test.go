...
func TestGramBuildDedupIsBitIdentical(t *testing.T) {
	...
	withDedup := bb.buildGrams(cells{flat: &deduped})
	...
	plain.forEachGram(&cells{flat: &every}, plain.distinct.Add)

	f := bloom.New(plain.distinct.Estimate(), gramFalsePositiveRate)
	plain.forEachGram(&cells{flat: &every}, f.Add)
...
func FuzzGramHintsAreColumnGrams(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, prefix, lit, suffix []byte) {
		...
		bb.forEachGram(&cells{flat: colOf(value)}, func(g []byte) { have[string(g)] = struct{}{} })
