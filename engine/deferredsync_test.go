...
func TestPartsSyncedBeforeIndexCommit(t *testing.T) {
	...
	be := &syncedCommits{Deferred: backendtest.WithDeferred(backend.Memory()), t: t, key: lostIndexKey()}
	e := newLostEngine(be)

	for i := range 3 {
		mustAppend(t, e, mkSeries("job", "api"), int64(100*(i+1)), float64(i))
		mustAppend(t, e, mkSeries("job", "db"), int64(100*(i+1)), float64(i))
