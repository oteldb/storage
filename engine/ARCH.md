...
**A logged write decides, logs, then applies.** With a WAL, `Append`, `AppendBatch` and
`ApplyPrimary` stage every sample in `walBatch` — admitted exactly as `appendByID` would admit it had
