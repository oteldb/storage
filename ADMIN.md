...
| `WantedParts` | repair obligations outstanding right now: parts this engine's index names but cannot read, awaiting a copy from a peer. Non-zero means the shard is **not fully repaired**; it clears when the parts come back or the losses are acknowledged. Reads over their range still answer from what is here |
