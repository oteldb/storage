...
| `IndexFenced` | this engine's last bucket-index load failed — the index was unreadable, or a part it names could not be opened for a reason other than being gone (a corrupt manifest, a backend error, a format this binary cannot read) — so it **refuses every commit**: flush, merge, retention, repair and `CompactNow` fail with an error wrapping `bucketindex.ErrFenced`. A commit from an engine whose view is not the stored index could drop every entry it never opened. Reads keep answering from the last part set that loaded. Flushes keep their records in the head, which therefore grows until `tenant.Limits.MaxInFlightBytes` sheds writes (without bound when it is zero, the default); the WAL keeps them durable meanwhile. Every maintenance cycle retries the load, and the first that succeeds clears this and lets the held-back flush commit. One that stays set is a part this node cannot read: it is not turned into a repair want, so it needs an operator (`index.fenced_loads` counts each failed attempt) |
...
| `index.fenced_loads` | `signal` | bucket-index loads that failed and left the engine refusing commits (`SignalStats.IndexFenced`). A fenced engine retries once per maintenance cycle and each failure counts again, so a steady rate is one engine that stays fenced; a single count that stops is a transient failure the next retry cleared. A corrupt artifact behind it also counts in `corruption.detected` with `disposition=fatal` |
