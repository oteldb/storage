...
func TestColumnScanRereadsOnBackwardSeek(t *testing.T) {
	...
	for _, g := range order {
		got, _, err := scan.DecodeBytesBlock(g)
		require.NoErrorf(t, err, "granule %d", g)
...
func TestColumnScanRejectsBytesBlockOfNumericColumn(t *testing.T) {
	...
	_, _, err = scan.DecodeBytesBlock(0)
	require.Error(t, err)
}
...
func TestDecodeBytesBlockRejectsBlockOutOfRange(t *testing.T) {
	...
	_, _, err = scan.DecodeBytesBlock(-1)
	require.Error(t, err)

	_, _, err = scan.DecodeBytesBlock(scan.NumBlocks())
	require.Error(t, err)
}
