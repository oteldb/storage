...
A table is named by a `DictGen` token, not by its address: `Decoder.SharedEntries` returns one token for
the decoder's life, `DecodeBytesBlock` returns it for a granule on the shared dictionary and a fresh
one for any other. `AppendDict` refuses a token other than the bound one, so a table from another
decoder or a stale granule is an error rather than a silent mis-map, even if a future decoder reuses
its entry slices. Bindings belong to one writer and die when it finishes.
...
- `Decoder.DecodeBytesBlock` decodes one granule against the cached frame. The result **aliases that
  ...
  nothing is rehashed per granule; any other granule gets a fresh token. Its `IDWidth` is the granule's own, so a trailer column hands back 1-byte
