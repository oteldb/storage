...
func buildAuto(t *testing.T, vals []float64, comp *compress.Compressor) (chunk.Codec, []float64) {
	...
	desc, obj, err := buildColumn(Column{Name: "v", Kind: KindFloat64, Float64: vals, AutoCodec: true}, comp, defaultGranuleSize, defaultCompressBlockBytes)
...
func TestAutoCodecConstantCollapses(t *testing.T) {
	...
	desc, obj, err := buildColumn(Column{Name: "v", Kind: KindFloat64, Float64: makeConstFloats(64, 7.5), AutoCodec: true}, noneComp(), defaultGranuleSize, defaultCompressBlockBytes)
	require.NoError(t, err)
...
func TestAutoCodecLossyBudget(t *testing.T) {
	...
	descLossless, objLossless, err := buildColumn(Column{Name: "v", Kind: KindFloat64, Float64: vals, AutoCodec: true}, noneComp(), defaultGranuleSize, defaultCompressBlockBytes)
	require.NoError(t, err)
	assert.Equal(t, uint8(0), descLossless.FloatPrecisionBits)

	descLossy, objLossy, err := buildColumn(Column{Name: "v", Kind: KindFloat64, Float64: vals, AutoCodec: true, FloatPrecisionBits: 12}, noneComp(), defaultGranuleSize, defaultCompressBlockBytes)
	require.NoError(t, err)
...
func TestAutoCodecLossyNonFiniteFallsBack(t *testing.T) {
	...
	desc, obj, err := buildColumn(Column{Name: "v", Kind: KindFloat64, Float64: vals, AutoCodec: true, FloatPrecisionBits: 12}, noneComp(), defaultGranuleSize, defaultCompressBlockBytes)
	require.NoError(t, err)
