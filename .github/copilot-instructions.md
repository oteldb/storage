...
- **`ADMIN.md`** — the operator/observability surface as built: `Inspect`/`StoreStats`, `Admin`,
  ...
  — a new or changed field/method on `Inspect`/`StoreStats`, `Admin`, `AdmissionStats`, `Parts*`, or
  `Cardinality`, or a new/renamed `obs` instrument — must update `ADMIN.md` in the same change.
...
- **Library, not a binary.** No `main`, no CLI, no auth, no scraping. The embedder (e.g. the
  ...
  `DESIGN.md` §5; keep everything else internal. **Exception: the L0 cluster layer
...
- **Follow the milestone order** in `DESIGN.md` §14. Don't build a layer before the one it
  depends on exists and is tested. `ARCHITECTURE.md` says which layers exist today.
