...
- **Sidecars take zstd at the default level; deltas stay raw.** The engine treats the side store as
  opaque and passes it no compressor, and its `MergeCompression` does not apply at flush anyway, so
  the table picks its own; the algorithm byte keeps the choice a writer decision. zstd-19 gains 1.5%
  over the default and `LevelFast` saves 10–20% encode time for 3% of the ratio. A delta
  (`Batch.Side`) is encoded per ingest batch and rides the WAL and replication, whose wire framing
  already compresses, so it is written `AlgorithmNone`.
- **The cost is encode time.** zstd encodes at 110–270 MB/s against 460–1190 MB/s raw, and decodes
  at 460–1035 MB/s against 600–1410. Flush and merge pay the encode once per part. A resolver build
  pays it too: `SideSnapshot` encodes the head and the union compresses its output, which
  `NewResolver` decodes straight back.
