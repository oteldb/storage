...
bench_set() {
  ...
  # The release-aware metric + record reads (realistic embedder pattern) — Batch.Release pooling.
  go test -run '^$' -bench '^(BenchmarkLogReadAll|BenchmarkLogReadRelease)$' -benchmem -count="$COUNT" -benchtime="$BENCHTIME" .
